...
// in simulator we redefine to format for udptest Developer console
...
{\
	NSString * exceptionName = [@"org.udptest." stringByAppendingString:NSStringFromClass([self class])];\
...
+ (void) throwException:(NSString *) reason subreason:(NSString*)subreason location:(NSString *)location\
{\
	NSString * exceptionName = @"org.udptest";\
	NSString * message = [NSString stringWithFormat:@"%@. %@ %@",reason,(subreason!=nil?subreason:@""),(location!=nil?location:@"")];\
	TiExceptionThrowWithNameAndReason(exceptionName,message);\
}\
...
enum {
	UIudptestNativeItemNone = -1, 
	UIudptestNativeItemSpinner = -2,
	UIudptestNativeItemProgressBar = -3,
	
	UIudptestNativeItemSlider = -4,
	UIudptestNativeItemSwitch = -5,
	UIudptestNativeItemMultiButton = -6,
	UIudptestNativeItemSegmented = -7,
	
	UIudptestNativeItemTextView = -8,
	UIudptestNativeItemTextField = -9,
	UIudptestNativeItemSearchBar = -10,
	
	UIudptestNativeItemPicker = -11,
	UIudptestNativeItemDatePicker = -12,
	
	UIudptestNativeItemInfoLight = -13,
	UIudptestNativeItemInfoDark = -14,
	
	UIudptestNativeItemDisclosure = -15,
	
	UIudptestNativeItemContactAdd = -16
};
...
/**
 *	TiThreadPerformOnMainThread should replace all udptest instances of
